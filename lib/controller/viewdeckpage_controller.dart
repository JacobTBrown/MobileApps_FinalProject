...
class ViewDeckPageController {

  ViewDeckPageState state;

  ViewDeckPageController(this.state);
  ...
  CardModel idDeckCheck(int index) {
    return state.storeCardList.firstWhere((card) {
      if (card.documentId == this.cardList[index].v)
        return true;
      else return false;
    });
  }
  ...
  void onTap(int index) {
    this.cardList.add(DeckEntry(state.user.deckNameList[state.indexX]+state.cardCount.toString(), state.user.cardsList[index]));
